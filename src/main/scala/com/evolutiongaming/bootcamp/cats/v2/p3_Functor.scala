...
object p3_Functor {
  ...
  val users = Map("Bob" -> User("Bob", 32), "Alice" -> User("Alice", 23), "Ann" -> User("Ann", 54))

  // The only thing we care about here is Functor instance for F to be able to call .map function.
  def getUserAge[F[_]: Functor](find: String => F[User])(name: String): F[Int] = find(name).map(_.age)

  def asyncFindUser(name: String): IO[User] = users.get(name) match {
    case Some(user) => IO.pure(user)
    case None       => IO.raiseError(new Throwable("Aw, snap!"))
  }

  def maybeUser(name: String): Option[User] = users.get(name)

  def userOrError(name: String): Either[String, User] = users.get(name) match {
